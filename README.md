...
| Component | Status |
|---|---|
| Agent Registry | **Native, and populated.** All four fleet agents are registered and discoverable in `agents.list` on a non-organization account ([ADR 0001](docs/adr/0001-governance-platform.md)), each with its skills and the route that actually reaches it. Registration is a committed, re-runnable script ([register_agents.py](scripts/register_agents.py)) that builds the catalog from the graph's own node constants, refuses to run if they drift, and refuses to register a route the deployed service does not serve. The live catalog is recorded in [DEPLOYMENT.md](docs/DEPLOYMENT.md). There is no `agents.create`: registration goes through `services.create` and the registry derives the agent record |
